...
class NatsConnectionManager(
    ...
    private val reconnectDuration: Duration = 30.seconds
        ...
        while (currentCoroutineContext().isActive) {
            ...
            when (val status = conn.status) {
                ...
                Connection.Status.RECONNECTING, Connection.Status.DISCONNECTED -> {
                    ...
                    if (reconnectDuration > Duration.ZERO &&
                        System.currentTimeMillis() - since >= reconnectDuration.inWholeMilliseconds
                    ) {
                        reconnectingSinceMillis = null
                        reconnectCompletely(
                            "connection stayed $status for at least $reconnectDuration"
                        )
                    }
                }
    ...
    companion object {
        private val logger = LogManager.getLogger(NatsConnectionManager::class.java)
    }
}